...
function AisEncode(msg) {
    ...
    switch (msg.aistype) {
        ...
        case 4: // Base Station Report
            this.class = 'B';
            this.PutInt(msg.year || 0, 38, 14); // Year
            this.PutInt(msg.month || 0, 52, 4); // Month
            this.PutInt(msg.day || 0, 56, 5); // Day
            this.PutInt(msg.hour || 0, 61, 5); // Hour
            this.PutInt(msg.minute || 0, 66, 6); // Minute
            this.PutInt(msg.second || 0, 72, 6); // Second
            this.PutInt(msg.accuracy || 0, 78, 1); // Position accuracy

            // Longitude handling
            lon = parseInt((msg.lon || 0) * 600000);
            if (lon < 0) lon |= 0x08000000;
            this.PutInt(lon, 79, 28);

            // Latitude handling
            lat = parseInt((msg.lat || 0) * 600000);
            if (lat < 0) lat |= 0x04000000;
            this.PutInt(lat, 107, 27);

            this.PutInt(msg.epfd || 0, 134, 4); // EPFD type
            this.PutInt(msg.raim || 0, 138, 1); // RAIM flag
            this.PutInt(msg.radio || 0, 139, 19); // Radio status

            this.payloadSize = 168;
            break;
        case 14:  // Broadcast Safety Related Message
            this.class = 'B';
            
            this.safetyMessage = msg.safetyMessage;
            this.PutStr(this.safetyMessage, 40, 161);
            this.payloadSize = 168;
            break;
        ...
        case 8: // Binary Broadcast Message
            this.class = 'B';
            this.PutInt(msg.dac, 38, 10); // Designated Area Code
            this.PutInt(msg.fid, 48, 6);  // Functional ID

            // Handle binary data payload
            if (msg.data && msg.data.length > 0) {
                for (var i = 0; i < msg.data.length; i++) {
                    // Encode each character to 6-bit
                    this.PutInt(msg.data.charCodeAt(i), 54 + i * 6, 6);
                }
            }

            this.payloadSize = 168; // Ukuran payload standar untuk tipe 8
            break;

        case 9: // Standard SAR Aircraft Position Report
            this.class = 'B';
            this.PutInt(parseInt(msg.alt || 0), 38, 12); // Altitude
            this.PutInt(parseInt((msg.sog || 0) * 10), 50, 10); // Speed over ground
            this.PutInt(parseInt(msg.accuracy || 0), 60, 1); // Position accuracy
            ...
            // Course over ground
            cog = parseInt((msg.cog || 0) * 10);
            this.PutInt(cog, 116, 12);

            // True heading (missing in previous code)
            this.PutInt(parseInt(msg.hdg || 0), 124, 9);

            this.PutInt(parseInt(msg.second || 60), 133, 6); // Time stamp (60 if not available)
            this.PutInt(parseInt(msg.dte || 0), 139, 1); // DTE flag
            this.PutInt(parseInt(msg.assigned || 0), 140, 1); // Assigned mode flag
            this.PutInt(parseInt(msg.raim || 0), 141, 1); // RAIM flag
            this.PutInt(parseInt(msg.radio || 0), 142, 19); // Radio status
        ...
        case 21: // Aid-to-Navigation Report
            this.class = 'B';
            this.PutInt(parseInt(msg.atonType || 0), 38, 5); // Type of AtoN
            this.PutStr(msg.name || "", 43, 120); // Name of AtoN

            // Position accuracy
            this.PutInt(parseInt(msg.accuracy || 0), 163, 1);
            ...
            // Dimensions
            this.PutInt(parseInt(msg.dimA || 0), 219, 9); // to bow
            this.PutInt(parseInt(msg.dimB || 0), 228, 9); // to stern
            this.PutInt(parseInt(msg.dimC || 0), 237, 6); // to port
            this.PutInt(parseInt(msg.dimD || 0), 243, 6); // to starboard

            this.PutInt(parseInt(msg.epfd || 0), 249, 4); // Type of EPFD
            this.PutInt(parseInt(msg.second || 60), 253, 6); // UTC second
            this.PutInt(parseInt(msg.off_position || 0), 259, 1); // Off position flag
            this.PutInt(parseInt(msg.regional || 0), 260, 8); // Regional reserved
            this.PutInt(parseInt(msg.raim || 0), 268, 1); // RAIM flag
            this.PutInt(parseInt(msg.virtual_aid || 0), 269, 1); // Virtual AtoN flag
            this.PutInt(parseInt(msg.assigned || 0), 270, 1); // Assigned mode flag

            this.payloadSize = 272;
            break;
...
// Fungsi konversi bilangan ke biner

// Prosedur bit stuffing
AisEncode.prototype.ApplyBitStuffing = function () {
    let consecutiveOnes = 0;
    for (let i = 0; i < this.bitstream.length; i++) {
        if (this.bitstream[i] === 1) {
            consecutiveOnes++;
            if (consecutiveOnes === 5) {
                // Sisipkan 0 setelah 5 consecutive 1's
                this.bitstream.splice(i+1, 0, 0);
                consecutiveOnes = 0;
            }
        } else {
            consecutiveOnes = 0;
